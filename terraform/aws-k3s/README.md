...
Esta opción de puntos extra crea VPC, subnet pública, Internet Gateway, rutas, Security Group y una instancia EC2 cifrada con k3s. Es más económica que EKS, pero sigue generando costes de EC2, EBS y dirección IPv4 pública.
...
```bash
AWS_PROFILE=csibol terraform init
AWS_PROFILE=csibol terraform plan -out=tfplan
AWS_PROFILE=csibol terraform apply tfplan
terraform output
```
...
Al terminar la evaluación, evitar costes residuales:

```bash
AWS_PROFILE=csibol terraform destroy
```
